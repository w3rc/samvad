...
- **Signing migrated to RFC 9421 (HTTP Message Signatures).** The `signature` and `kid` fields are removed from the request envelope body. The signature, key ID, and body hash are now carried in three standard HTTP headers: `Signature`, `Signature-Input`, and `Content-Digest`. See §5.1 and §7-L2 for the updated wire format.
...
The JSON body contains the routing and payload fields. Authentication is carried in HTTP headers (see §7-L2).
...
**Signature is carried in HTTP headers — not in the body.** See §7-L2 for the RFC 9421 header format and signing procedure.
...
### L2 — Message Signing (RFC 9421 HTTP Message Signatures)

Every request is signed using **RFC 9421 (HTTP Message Signatures)** with Ed25519. The signature, key ID, and body hash are carried in three standard HTTP headers:
...
1. Serialize the JSON body to UTF-8 bytes with no added whitespace
2. Compute `Content-Digest: sha-256=:base64(sha256(body)):`
3. Construct the RFC 9421 signature base:
   ```
   "@method": POST
   "@path": /agent/message
   "content-digest": sha-256=:hash:
   "@signature-params": ("@method" "@path" "content-digest");keyid="key-2";alg="ed25519";created=1744371600
   ```
   (Each line is terminated by `\n`, including the final `@signature-params` line.)
4. Sign the UTF-8-encoded signature base with Ed25519
5. Attach `Content-Digest`, `Signature-Input`, and `Signature` headers to the HTTP request

**Verification procedure:**

1. Recompute `sha-256` of the raw request body and compare to `Content-Digest` — reject if different
2. Read `keyid` from `Signature-Input`; look up the sender's public key from their agent card
3. Reconstruct the signature base using the request's method, path, and `Content-Digest` header
4. Verify the Ed25519 signature in `Signature` against the reconstructed base and public key

Messages older than 5 minutes or whose nonce has already been seen within the window are rejected before signature verification (§5.1 `nonce` + `timestamp` fields).
...
Agent cards list every active and inactive key with a `kid`. The `keyid` parameter in `Signature-Input` identifies which key was used to sign. Receivers re-fetch the card after `cardTTL` seconds, so deactivating a key propagates globally with no central revocation server.
