    ...
    *,
    ...
    async def verify_incoming(
        ...
        try:
            ...
            return VerifyResult(
                ok=False, error=SamvadError(ErrorCode.SCHEMA_INVALID, "invalid timestamp format")
            )

        nonce_fresh = await nonce_store.check(sender, envelope.nonce, ts_unix)
        if not nonce_fresh:
            return VerifyResult(
                ok=False, error=SamvadError(ErrorCode.REPLAY_DETECTED, "replay or expired")
            )
        ...
        if not rate_limiter.check_request(sender):
            ...
            return VerifyResult(
                ok=False, error=SamvadError(ErrorCode.RATE_LIMITED, "rate limited")
            )
        ...
        if pub_key is None:
            return VerifyResult(
                ok=False, error=SamvadError(ErrorCode.AUTH_FAILED, f"unknown peer: {sender}")
            )
        ...
        if not verify_request(method, path, lower, pub_key):
            return VerifyResult(
                ok=False,
                error=SamvadError(ErrorCode.AUTH_FAILED, "signature verification failed"),
            )
        ...
        if envelope.delegation_token is not None:
            ...
            try:
                ...
                return VerifyResult(
                    ok=False,
                    error=SamvadError(ErrorCode.AUTH_FAILED, "invalid delegation token format"),
                )
            ...
            if issuer_pub_key is None:
                return VerifyResult(
                    ok=False,
                    error=SamvadError(
                        ErrorCode.AUTH_FAILED, f"delegation issuer unknown: {issuer}"
                    ),
                )
            ...
            try:
                claims = _verify_token(
                    envelope.delegation_token, issuer_public_key_b64=issuer_pub_key
                )
            ...
            if claims.get("sub") != sender:
                return VerifyResult(
                    ok=False,
                    error=SamvadError(
                        ErrorCode.AUTH_FAILED, "delegation token sub does not match sender"
                    ),
                )
            if skill_name not in claims.get("scope", []):
                return VerifyResult(
                    ...
                    error=SamvadError(
                        ...
                        f"skill '{skill_name}' not in delegation scope",
                    ),
                )
        ...
        if scan_object_for_injection(payload):
            return VerifyResult(
                ok=False,
                error=SamvadError(ErrorCode.INJECTION_DETECTED, "injection pattern detected"),
            )
            ...
            try:
                ...
                if detected:
                    return VerifyResult(
                        ok=False,
                        error=SamvadError(
                            ErrorCode.INJECTION_DETECTED, "classifier detected injection"
                        ),
                    )
            except Exception:
        ...
        if skill is None:
            return VerifyResult(
                ok=False,
                error=SamvadError(ErrorCode.SKILL_NOT_FOUND, f"unknown skill: {skill_name}"),
            )
        ...
        if trust == "authenticated":
            ...
            if not (envelope.auth and envelope.auth.token):
                return VerifyResult(
                    ok=False, error=SamvadError(ErrorCode.AUTH_FAILED, "Bearer token required")
                )
        if trust == "trusted-peers":
            ...
            if sender not in allowed:
                return VerifyResult(
                    ok=False,
                    error=SamvadError(ErrorCode.AUTH_FAILED, "not in trusted-peers list"),
                )
